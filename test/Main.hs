...
-- initial generator derived from it. A run is therefore reproducible even
...
main = do
  ...
  ConnectionSource connSource <- poolSource connSettings $ \connect disconnect ->
  ...
  let allTests = askOption $ \(Seed mseed) ->
        ...
        in testGroup "hpqtypes" $ tests (TestData gen connSettings connSource)
