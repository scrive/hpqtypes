...
-- | Deliberately has no 'FromSQL' instance. 'genericDecoder' decodes it.
...
decoderErrorTest td =
  ...
  where
    -- The library compares the number of rows with the expectation before it
    -- decodes any of them. It therefore reports a result of the wrong size as
    -- such, not as whatever error the decoding of its first rows produces.
    rowCountCheckedFirst = testCase "Row count is checked before decoding" $ do
      runTestEnv td defaultTransactionSettings $ do
        ...
        -- The rows don't match the decoder either. Old versions reported that
        -- mismatch instead.
    ...
    -- Documented behavior of decodeNullable: the given decoder doesn't run on
    -- NULL, so it doesn't compare the type either.
...
rowDecoderTest td =
  ...
  where
    ...
    namedCompositeWorks = testCase "Named composite decoding works" $ do
      runTestEnv td defaultTransactionSettings $ do
        -- The casts exercise decoding of user-defined composite types. Their
    ...
    differentCompositeFails = testCase "Field type mismatch in a composite is detected" $ do
      runTestEnv td defaultTransactionSettings $ do
        ...
        -- types of Simple (int4, date). The decoder of the first field detects
        -- the mismatch.
