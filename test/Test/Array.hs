...
arrayDecoderTest td =
  ...
  where
    ...
    -- Every built-in scalar type routes through 'decodeScalarArray', the fast
    -- path for arrays of scalars. It must report the position of a failing
    -- element like the general path does. Without the position, there is no
    -- way to tell which element of a large array is at fault.
    elementErrorsCarryPosition =
      ...
      where
        ...
        -- The array is the first column of the result and the offending
        -- element is the second element of the array. The error of the array
        -- field wraps the error of the element.
    ...
    malformedDimensionsFail = testCase "Malformed array dimensions are rejected" $ do
      runTestEnv td defaultTransactionSettings $ do
        -- A header that claims dimensions of sizes 10^9 and 0. The number of
        -- elements is 0, so the lack of element data is consistent. If the
        -- decoder accepted the sizes, it would allocate 10^9 sub-arrays.
        let zeroDim =
              BS.pack $
                concat
        ...
        expectError @HPQTypesError "dimension of size 0" (\_ -> pure ()) $
          fetchOne (fromSQL @[[Int32]])
        -- A header that claims three dimensions of size 2^32 - 1. The number
        -- of elements exceeds the maximum the server can send and overflows
        -- Int64.
        let hugeDims =
              BS.pack $
                concat
    ...
    raggedArraysAreRejected = testCase "Ragged multi-dimensional arrays are rejected before sending" $ do
      runTestEnv td defaultTransactionSettings $ do
        ...
        -- taken from the last sub-list. Without the client-side comparison, the
        ...
        -- Raggedness visible only across branches. The number of elements
        -- matches the dimensions taken from the last branch ([3, 2, 3]).
        -- Sub-arrays are uniform among their immediate siblings, so only a
        -- comparison of their full dimensions catches this silent reshape.
        rejected
        ...
        -- A NULL sub-array. The server cannot represent it either.
