...
enumTest td = testCase "Enum deriving helpers work"
  ...
  $ do
    ...
    -- The MonadThrow instance of RowDecoder wraps the RangeError in a
    -- ConversionError with the position of the field.
    runSQL_ "SELECT 42::int2"
    expectError @ConversionError "enum error carries position" checkRangePosition $
      fetchOne (fromSQL @Colours)

    -- Values of actual PostgreSQL enum types work in inserts, comparisons and
    -- fetches without a cast.
    ...
    -- The library sends arrays of text enums as text[]. A context without type
    -- information works, but an enum column needs a cast.
