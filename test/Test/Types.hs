...
typesTests td =
  ...
  , putGetTest @JSONBValue0 td 50
  , putGetTest @RawJSON td 50
...
integerTest td = testCase "Integer decoding from numeric works"
  ...
  $ \n -> do
    ...
    -- representation of numeric. A multiple of 10000 therefore arrives with
    -- fewer digits than its weight indicates.
...
jsonTest td = testCase "JSON conversion failures and raw values"
  ...
  $ do
    ...
    expectError @HPQTypesError "json as Int64" checkAesonError $
      fetchOne (fromSQL @(JSON Int64))
    runSQL_ "SELECT '\"str\"'::jsonb"
    expectError @HPQTypesError "jsonb as Int64" checkAesonError $
      fetchOne (fromSQL @(JSONB Int64))
    ...
    (rawJson, rawJsonb) <- fetchOne ((,) <$> fromSQL <*> fromSQL)
    assertEqual "json text is verbatim" (RawJSON "{\"b\": 1,  \"a\": 2}") rawJson
    assertEqual "jsonb text is normalized" (RawJSONB "{\"a\": 2, \"b\": 1}") rawJsonb

    -- The helpers convert between a raw value and a type with aeson instances.
    let object = KM.fromList [("a", Number 2), ("b", Number 1)]
    assertEqual "raw json decodes" (Right $ Object object) $ eitherDecodeRawJSON rawJson
    assertEqual "raw jsonb decodes" (Just $ Object object) $ decodeRawJSONB rawJsonb
    assertEqual "json encodes" (RawJSON "{\"a\":2,\"b\":1}") $ encodeRawJSON object
    assertEqual "jsonb encodes" (RawJSONB "{\"a\":2,\"b\":1}") $ encodeRawJSONB object
  where
    checkAesonError (HPQTypesError msg) =
      liftBase . assertBool ("Error names the type and comes from the FromJSON instance: " ++ msg) $
        "cannot decode JSON as Int64" `L.isInfixOf` msg
          && "expected Number, but encountered String" `L.isInfixOf` msg
...
-- netmask. A bare 'IP' can therefore only represent a value whose netmask
-- covers the whole address.
inetTest :: TestData -> TestTree
inetTest td = testCase "inet maps to IP and cidr to IPRange"
  . runTestEnv td defaultTransactionSettings
  $ do
    -- Old versions discarded the host bits, which turned this into 10.0.0.0/8.
    ...
    -- The decoder rejects a netmask narrower than the address instead of
    -- dropping it.
    runSQL_ "SELECT '10.0.0.5/8'::inet"
    expectError @HPQTypesError "netmask on an IP" checkNetmask $ fetchOne (fromSQL @IP)

    -- A bare address has a netmask that covers it, so it decodes.
...
-- enough exponent doesn't fit. The upstream encoder lets such a value wrap
-- around, which silently stores a different number. @1e131072@ used to
-- arrive as @0@.
...
-- has those plus @NaN@. The Haskell types they map to cannot represent any
-- of them. The library rejects them in both directions instead of silently
-- turning them into a value that means something else. On the wire the
-- infinities are the extremes of the representation. If the decoder accepted
-- them, it would yield a date far outside the range the server accepts. If
-- the encoder accepted a value that lands on one, it would send an infinity.
...
infiniteValuesTest td = testCase "Values with no Haskell representation are rejected"
  ...
  where
    ...
    -- The values whose wire representation stands for infinity.
...
-- | The length prefix of a value inside a container is signed, and @-1@
-- stands for NULL. The upstream decoder passes any other negative length on
-- to @unsafeTake@ / @unsafeDrop@. The result is a 'BS.ByteString' of negative
-- length that points before its buffer.
  ...
  case D.valueParser D.int4range negativeLength of
    ...
    Left err ->
      HUnit.assertBool ("Error mentions the length: " ++ T.unpack err) $
        "-24" `T.isInfixOf` err
  where
    -- Flags (lower and upper inclusive, neither infinite), then a bound that
    -- claims a length of 0xffffffe8, i.e. -24.
...
-- | The server rejects NUL characters in values of the character types. The
-- encoders must therefore pass them on verbatim. The upstream encoders in
-- @postgresql-binary@, from which the ones in
-- "Database.PostgreSQL.PQTypes.Internal.Encoding" derive, silently drop such
-- characters. This corrupts the value instead.
...
nulInTextTest td = testCase "NUL characters in text values are rejected"
  ...
  $ do
    ...
    -- The XML parser rejects the character before the encoding does.
  ...
  where
    ...
    rejected what expected row = do
      -- The receive function of the type of a parameter runs when the server
      -- binds the query, so the query itself doesn't need to use the value.
      -- The savepoint keeps the transaction usable for the cases that follow.
...
-- | 'Int' and 'Word' cannot be decoded, because their 'FromSQL' instances use
-- TypeError. The test therefore fetches the encoded values back as 'Int64'
-- and 'Word64'.
...
rangeTest td = testCase "Range decoding works" $ do
  runTestEnv td defaultTransactionSettings $ do
    -- The server canonicalizes the first range to [2,6).
    runSQL_ $
      mconcat
