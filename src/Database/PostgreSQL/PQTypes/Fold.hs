...
-- | Get the current t'QueryResult'. If there is none, throw an exception.
queryResult
...
-- | Specialization of 'foldlDB' that fetches one or zero rows. If the result
-- has more rows, it throws t'AffectedRowsMismatch'.
--
-- The function compares the number of rows before it decodes any of them. It
-- reports a result of the wrong size as such even if the rows don't match the
-- decoder either.
fetchMaybe
  :: (HasCallStack, MonadDB m, MonadThrow m)
  => RowDecoder a
...
-- | Specialization of 'foldlDB' that fetches exactly one row. If the result
-- has no row or more rows, it throws t'AffectedRowsMismatch'.
--
-- The function compares the number of rows before it decodes any of them. It
-- reports a result of the wrong size as such even if the rows don't match the
-- decoder either.
