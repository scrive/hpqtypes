...
--
-- t'SQLEnum' and t'SQLEnumAsText' use it to derive SQL instances.
class (Ord base, Bounded a, Enum a) => EnumEncodingAs base a | a -> base where
  -- | Encode @a@ as the base type.
  encodeEnumAs :: a -> base

  -- | The inverse map is a top-level part of the 'EnumEncodingAs' instance
  -- so that GHC computes it only once.
  enumEncodingMap :: M.Map base a
  enumEncodingMap = M.fromList [(encodeEnumAs a, a) | a <- enumerate]

----------------------------------------

-- | Helper newtype for @deriving via@. It derives @(PQFormat, ToSQL,
-- FromSQL)@ instances for an enum from its 'EnumEncodingAs' instance.
--
-- /Hint:/ the 'generic-data' package derives non-trivial 'Enum' instances.
--
-- >>> :{
...
instance
  ...
  fromSQL = do
    ...
    where
      -- The keys of the map are ascending and duplicate-free.
...
-- of actual PostgreSQL enum types. t'SQLEnum' can't decode them, because it
-- requires an 'Enum' instance of the base type, which 'Text' doesn't have.
--
-- Encoded values are sent with their type unspecified, so the server infers
-- it from the context. They work directly against both text and enum
-- columns. The flip side is that they don't work in a context that provides
-- no type information, e.g. @SELECT $1@ alone fails with \"could not
-- determine data type of parameter\".
--
-- Lists and 'Data.Vector.Vector's of such values are sent as @text[]@,
-- because a binary array parameter carries a concrete element type that the
-- server cannot infer. They work directly against @text@ columns. A column of
-- an actual enum type requires a cast, e.g. @... = ANY($1::my_enum_type[])@.
-- The cast is a genuine conversion, because the parameter is a @text[]@
-- value.
--
-- Decoding differs from t'SQLEnum' @a@ with an @'EnumEncodingAs' 'Text' a@
-- instance. Values are decoded with 'decodeEnum', which accepts both text and
-- PostgreSQL enum types. Errors are reported as t'InvalidValue' with the list
-- of valid labels, not as t'RangeError'.
--
-- >>> :{
...
-- | Doctests use it to prove the injectivity of encoding functions.
--
