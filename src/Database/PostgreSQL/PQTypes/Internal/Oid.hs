...
-- | Internal object identifier that PostgreSQL uses, in particular to
-- identify the types of query parameters and result columns.
...
-- OIDs of built-in types are stable. The values below come from pg_type.dat
...
-- | If a query parameter has this type, the server infers the type of the
