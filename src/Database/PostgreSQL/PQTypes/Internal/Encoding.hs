...
--
-- The encoders derive from the @postgresql-binary@ package by Nikita Volkov
...
--   silently drops NUL characters from it, which corrupts the value. The
--   server rejects such characters itself, with @22021
...
--   behavior and produces the wrong bytes on a big-endian architecture.
--
...
--   around and silently encodes a different number (@1e131072@ used to
--   arrive as @0@).
--
-- * The date and time encoders reject a value that doesn't fit the wire
--   format. They also reject a value whose representation stands for an
--   infinity of the type. Upstream truncates it and silently sends a
--   different date, or @infinity@ for two of the types.
--
-- * The time encoders take the values apart with the accessors of the
--   "Data.Time" and "Data.Fixed" types, not with a coercion to their
--   representations.
--
-- * 'inet' takes the address and the length of its netmask. The caller maps
--   a Haskell type onto them. Upstream takes an 'IP.IPRange', which cannot
--   carry the host bits of the address.
--
-- * t'Encoding' is a newtype, not an alias of the underlying builder, and
--   'array' takes an t'Oid', not a bare 'Word32'. Neither the builder
--   library nor a raw OID leaks into the public API of the library.
...
-- The length prefix is not part of it and travels out of band.
newtype Encoding = Encoding B.Builder
  deriving newtype (Monoid, Semigroup)

-- | Render an t'Encoding' as the bytes that go to the server.
...
-- | Prefix a payload with its length. The elements of arrays and the bounds
-- of ranges travel this way on the wire.
...
-- | Reject a value that the wire format cannot hold. Either the value doesn't
-- fit at all, or its representation stands for an infinity of the type.
...
array elemOid (Array payload dims nulls) =
  ...
  where
    -- The encoder sends every dimension with a lower bound of 1.
...
-- | An additional dimension of an array, given a fold over the container
-- that holds its elements.
...
numeric value
  ...
  where
    ...
    -- Digit groups hold four decimal digits each, so the exponent must be a
    -- multiple of four. The scaling of the coefficient makes it one.
...
-- microseconds within it. The arithmetic runs on t'Integer', because the
-- result overflows t'Int64' well before the day overflows t'Int32'.
...
-- of both @inet@ and @cidr@. The OID of the parameter sets the flag that
-- tells them apart, so the encoder always produces @inet@ here. The server
-- accepts that for a @cidr@ parameter if no bits are set to the right of
-- the netmask. That condition defines a @cidr@ value.
...
-- | Encode a value as @json@ with its 'A.ToJSON' instance.
json_aeson :: A.ToJSON a => a -> Encoding
json_aeson = json_bytes_lazy . A.encode

-- The wire format of jsonb is its JSON text with a version number prefix.
...
-- | Encode a value as @jsonb@ with its 'A.ToJSON' instance.
jsonb_aeson :: A.ToJSON a => a -> Encoding
jsonb_aeson = jsonb_bytes_lazy . A.encode
