...
--
-- The time related instances use encoders for servers that have the
-- @integer_datetimes@ parameter set to @on@. This is the case for all
-- supported versions of PostgreSQL, because PostgreSQL 10 removed the option
-- to compile the server with floating point datetimes.
class PQFormat a => ToSQL a where
  ...
  -- same type regardless of its number of dimensions, so the innermost
  -- elements determine its type.
  arrayElemOid :: Oid
  arrayElemOid = pqOid @a

  -- | Encode a value of this type as an element of an array. For most types
  -- this is the encoding of the value itself, with 'Nothing' as a NULL
  -- element. Lists and 'V.Vector's instead become an additional dimension
  -- of the enclosing array.
  arrayElem :: a -> E.Array
  arrayElem = maybe E.nullArray E.encodingArray . toSQL

  -- | 'toSQL' of @[a]@: the encoding of an array by default. 'Char'
  -- overrides it so that 'String' is encoded as @text@. This is why the
  -- 'ToSQL' instance for lists cannot define its methods directly.
  toSQLList :: [a] -> Maybe E.Encoding
  toSQLList = Just . E.array (arrayElemOid @a) . arrayElemList

  -- | 'arrayElemOid' of @[a]@: passed through by default, because an array
  -- is of the same type regardless of its number of dimensions. 'Char'
  -- overrides it with the OID of @text@.
  arrayElemOidList :: Oid
  arrayElemOidList = arrayElemOid @a

  -- | 'arrayElem' of @[a]@: an additional dimension of the enclosing array
  -- by default. 'Char' overrides it with the encoding of the string as
  -- @text@.
  ...
  -- 'Nothing' for scalars, @Just f@ for lists and 'V.Vector's, which become
  -- an additional dimension of the enclosing array instead of elements.
  -- @f@ walks down the first branch of the value and collects lengths:
  --
  -- > f [[1,2,3], [4,5,6]]  =  [2,3]
  -- > f [[1,2,3], [4]]      =  [2,3]
  --
  -- @f@ inspects only the first branch. As the second example shows, the
  -- result describes the other branches only if the value is rectangular.
  -- @raggedDims@ uses it to make sure that this is the case.
  arrayElemDims :: Maybe (a -> [Int])
  arrayElemDims = Nothing

  -- | 'arrayElemDims' of @[a]@: an additional dimension of the enclosing
  -- array by default. 'Char' overrides it, because 'String' is a scalar
  -- @text@ element, so its length is not a dimension.
  arrayElemDimsList :: Maybe ([a] -> [Int])
  arrayElemDimsList = Just $ \xs -> length xs : elemDims xs

  -- The defaults that do per-element work are inlined so that instances in
...
-- | Make sure that all elements of an array have the same dimensions
-- ('arrayElemDims'). If they don't, return the first differing pair. Scalar
-- elements have no dimensions, so the result is 'Nothing' right away.
--
...
-- sequence. Without this function, @[[1,2], [3]]@ is sent as a 2x1 array
-- with 3 elements, because the dimensions come from the last sub-array. The
-- server rejects it with an obscure error. @[[1,2,3], [4], [5,6]]@ is sent
-- as a 3x2 array with 6 elements. The server accepts it and silently stores
-- @[[1,2], [3,4], [5,6]]@.
--
-- A comparison of the first-branch dimensions of the immediate elements is
-- enough, because encoding runs this function at every level of a nested
-- array (see 'arrayElemList' and 'arrayElem' of 'V.Vector'):
--
-- * @[[[1,2], [3,4,5]]]@: the outer array passes, because it has only one
--   element to take dimensions of. Encoding that element runs the function
--   on @[[1,2], [3,4,5]]@, which compares @[2]@ with @[3]@ and fails.
--
-- * @[[[1,2]], [[3,4,5]]]@: the inner arrays pass, because each has one
--   element. The outer array compares their dimensions @[1,2]@ with @[1,3]@
--   and fails.
--
-- In general, if every level passes, then at every level every element has
-- the same dimensions as the first branch, i.e. the whole array is
-- rectangular.
...
-- the 'Char' instance can override them. This way 'String' encodes as @text@
...
-- 'arrayElemOid' passes the OID of the innermost elements through to its top
-- level.
...
-- The "char" type stores a single byte. Characters above '\255' are not
