...
-- has no fixed number of days, and a day has no fixed number of hours.
--
-- The type is opaque. Construct values with the 'iyears', 'imonths' etc.
-- functions and combine them with the 'Monoid' instance, e.g.
-- @idays 2 <> ihours 3@.
data Interval = Interval
  ...
  , intMonths :: !Int32
  , intEstimate :: !Integer
  -- ^ Cached estimate that the server bases its comparison of intervals on,
  -- with a month converted at 30 days and a day at 24 hours. 'Eq' and 'Ord'
  -- use it. Always construct through 'mkInterval' to keep it in sync with
  -- the components.
  }

-- | Smart constructor that computes the cached estimate.
...
-- | Compares the same way as the comparison operators of the server, i.e. by
-- the estimate with a month converted at 30 days and a day at 24 hours. In
-- particular @imonths 1 == idays 30@ holds, just like @1 month = 30 days@
-- does on the server. The values are distinct nonetheless, e.g. adding them
...
-- | Whether two intervals consist of the same components, as opposed to
-- merely being equal.
--
-- 'Eq' compares intervals the way the comparison operators of the server do,
-- so it holds for values that the server nonetheless treats differently in
-- arithmetic. @'imonths' 1 == 'idays' 30@, yet adding the former to
-- @2024-01-31@ gives @2024-02-29@ and the latter @2024-03-01@. Use this
-- function to tell such values apart, e.g. to make sure that a roundtrip
-- through the database preserved an interval exactly.
...
-- | Shows the components of the wire format and omits the cached estimate.
...
-- encoder and decoder of postgresql-binary are of no use here, because
...
instance FromSQL Interval where
  fromSQL = decodeScalar . D.fn . BP.run $ do
    ...
    where
      -- An infinity has all three components at the extreme of their range.
      -- 'Interval' has no value that stands for it.
...
instance ToSQL Interval where
  toSQL value@Interval {..}
    -- If sent, such a value would arrive at the server as an infinity. It's
    -- rejected the way the date and time encoders reject the representations
    -- of theirs.
