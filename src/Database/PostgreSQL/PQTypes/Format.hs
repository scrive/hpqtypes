...
--
-- /Warning:/ the OIDs belong to the type, not to any of its values, so they
-- don't survive a coercion. If an instance gives a type different OIDs than
-- the type it wraps, you can't derive 'Database.PostgreSQL.PQTypes.ToSQL.ToSQL'
-- and 'Database.PostgreSQL.PQTypes.FromSQL.FromSQL' from it with
-- @GeneralizedNewtypeDeriving@, nor with @deriving via@ the wrapped type,
-- which is the same thing for a newtype. The derived instances keep the OIDs
-- of the wrapped type. Decoding then fails with
-- 'Database.PostgreSQL.PQTypes.Internal.Error.TypeMismatch', and the server
-- rejects arrays for the wrong element type. Write the instances out instead.
-- Deriving them is safe only if the OIDs match, i.e. if this class is derived
-- along with them.
class PQFormat a where
  ...
  -- | 'pqOid' of @[a]@. The default is the array type of @a@. 'Char'
  -- overrides it so that 'String' corresponds to @text@.
  pqListOid :: Oid
  pqListOid = pqArrayOid @a

  -- | 'pqArrayOid' of @[a]@. The default is 'pqArrayOid' itself, because an
  -- array is of the same type regardless of its number of dimensions. 'Char'
  -- overrides it so that @[String]@ corresponds to @text[]@.
  pqListArrayOid :: Oid
...
-- The OIDs of @[a]@ go through class methods so that the 'Char' instance can
-- override them. This makes 'String' correspond to @text@ instead of an array
